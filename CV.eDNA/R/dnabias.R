...
#' @param og_levels The original taxonomic ranks of the specimen classifications.
...
#' @param hierarchy A hierarchy data frame. This can be generated using the `hierarchy` function.
#' @return A list with refined classifications and levels.
#' @import dplyr
#' @export

dnabias = function(dna_df, og_classes, og_levels, samples, agreed, hierarchy){
  ...
  for(x in 1:length(samples)){
    ...
    if(agreed[x]){
      ...
      common_group = which(lengths(apply(sub_edna_a, 2, unique)) == 1
                           & !apply(sub_edna_a, 2, unique) %in% c(NA, "NULL", "indet."))
    ...
    # If the DNA detections do not agree with the presence of the detected class:
      ...
      if(sum(agree_levels) > 0){
        ...
        common_group = which(lengths(apply(sub_edna_d, 2, unique)) == 1
                             & !apply(sub_edna_d, 2, unique) %in% c(NA, "NULL", "indet."))
